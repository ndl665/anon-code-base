...
# 读取数据
train_data = read_answers('../../dataset/128/train128_1.json')
...
def train(model, train_data_loader, valid_data_loader, test_data_loader):
    ...
    # 训练结束后，直接使用当前（训练效果差的）模型进行测试
    print("\nTesting with the model from the last epoch.")
...
# 开始训练
# 每次运行脚本，都会从本地路径重新加载初始 CodeBERT 模型并开始训练。
