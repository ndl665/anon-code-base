    ...
    with open(filename) as f:
        for line in f:
            ...
            for i in range(len(js)):
                # print(js[i])
                example = InputExample(guid=js[i]['target'], text_a=js[i]['code1'], text_b=js[i]['func'])
...
train_dataset = read_answers('../../dataset/fewshot/train16_1.json')
valid_dataset = read_answers('../../dataset/val12.json')
test_dataset = read_answers('../../dataset/test12.json')
...
promptTemplate = MixedTemplate(
    model=plm,
    text='The clean code is {"placeholder":"text_a"}. {"soft"} The buggy code is {"placeholder":"text_b"}. {"soft"} The result is {"mask"}.',
    tokenizer=tokenizer,
)
